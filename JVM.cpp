...
JVM::JVM(int argc, const char* argv[])
{
    Init(argc, argv);
}
...
{
#if defined(_WIN32) || defined(_WIN64)
    this->module = LoadLibraryW(L"C:/Program Files/Java/jre1.8.0_73/bin/server/jvm.dll"); //C:/Program Files/Java/jre1.8.0_73/bin/server/jvm.dll
#else
    ...
    if (argc > 0)
    {
        ...
        if (JNI_GetCreatedJavaVMs(vms, max_vms, &num_vms) == JNI_OK)
        {
            for (int i = 0; i < num_vms; ++i)
            {
                ...
                {
                    this->vm = vms[i];
                    this->vm->AttachCurrentThread(reinterpret_cast<void**>(&this->env), NULL);
                    return result;
                }
            }
        }
        
        if (JNI_CreateJavaVM(&this->vm, reinterpret_cast<void**>(&this->env), &jvm_args) != JNI_OK)
        {
            result = false;
            this->vm = nullptr;
            this->env = nullptr;
...
int InputStream::available()
{
    ...
    return vm->CallIntMethod(this->inst, method);
}
...
bool InputStream::markSupported()
{
    ...
    return vm->CallBooleanMethod(this->inst, method);
}
...
void OutputStream::write(unsigned char* b, int bufferLength)
{
    jbyteArray arr = this->vm->NewByteArray(bufferLength);
    this->vm->SetByteArrayRegion(arr, 0, bufferLength, reinterpret_cast<jbyte*>(b));
    jmethodID method = this->vm->GetMethodID(this->cls, "write", "([b)V");
    this->vm->CallVoidMethod(this->inst, method, arr);
    this->vm->DeleteLocalRef(arr);
}

void OutputStream::write(unsigned char* b, int bufferLength, int off, int len)
{
    jbyteArray arr = this->vm->NewByteArray(bufferLength);
    this->vm->SetByteArrayRegion(arr, 0, bufferLength, reinterpret_cast<jbyte*>(b));
    jmethodID method = this->vm->GetMethodID(this->cls, "write", "([bIII)V");
    this->vm->CallVoidMethod(this->inst, method, arr, off, len);
    this->vm->DeleteLocalRef(arr);
}

void OutputStream::write(int b)
{
    jmethodID method = this->vm->GetMethodID(this->cls, "write", "(I)V");
    this->vm->CallVoidMethod(this->inst, method, b);
}
...
ByteArrayInputStream::ByteArrayInputStream(JVM* vm, unsigned char* buffer, int bufferLength)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    }
    
    this->vm = vm;
    jbyteArray arr = vm->NewByteArray(bufferLength);
    vm->SetByteArrayRegion(arr, 0, bufferLength, reinterpret_cast<const jbyte*>(buffer));
    
    this->cls = vm->FindClass("java/io/ByteArrayInputStream");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "([B)V");
    jobject localInstance = vm->NewObject(this->cls, method, arr);
    vm->DeleteLocalRef(arr);
    this->inst = vm->NewGlobalRef(localInstance);
}

ByteArrayInputStream::ByteArrayInputStream(JVM* vm, unsigned char* buffer, int bufferLength, int offset, int length)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    this->cls = vm->FindClass("java/io/ByteArrayInputStream");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "([BII)V");
    jobject localInstance = vm->NewObject(this->cls, method, arr, offset, length);
    vm->DeleteLocalRef(arr);
    this->inst = vm->NewGlobalRef(localInstance);
}
...
ByteArrayOutputStream::ByteArrayOutputStream(JVM* vm)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    this->vm = vm;
    this->cls = vm->FindClass("java/io/ByteArrayOutputStream");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "()V");
    jobject localInstance = vm->NewObject(this->cls, method);
    this->inst = vm->NewGlobalRef(localInstance);
}

ByteArrayOutputStream::ByteArrayOutputStream(JVM* vm, int size)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    this->vm = vm;
    this->cls = vm->FindClass("java/io/ByteArrayOutputStream");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "(I)V");
    jobject localInstance = vm->NewObject(this->cls, method, size);
    this->inst = vm->NewGlobalRef(localInstance);
}
...
{
    jmethodID method = this->vm->GetMethodID(this->cls, "size", "()I");
...
std::vector<unsigned char> ByteArrayOutputStream::toByteArray()
{
    ...
    jmethodID method = this->vm->GetMethodID(this->cls, "toByteArray", "()[B");
    jbyteArray bytes = reinterpret_cast<jbyteArray>(this->vm->CallObjectMethod(this->inst, method));
    result.resize(this->vm->GetArrayLength(bytes));
    void* arr = this->vm->GetPrimitiveArrayCritical(bytes, nullptr);
    memcpy(&result[0], arr, result.size());
...
{
    jmethodID method = this->vm->GetMethodID(this->cls, "toString", "()Ljava/lang/String;");
    jstring localInstance = reinterpret_cast<jstring>(this->vm->CallObjectMethod(this->inst, method));
    const char *nativeString = this->vm->GetStringUTFChars(localInstance, 0);
    std::string result = std::string(nativeString);
    this->vm->ReleaseStringUTFChars(localInstance, nativeString);
    return result;
}

void ByteArrayOutputStream::writeTo(OutputStream* out)
{
    jmethodID method = this->vm->GetMethodID(this->cls, "writeTo", "(Ljava/io/OutputStream;)V");
    this->vm->CallVoidMethod(this->inst, method, out->inst);
}
...
ZipEntry::ZipEntry(JVM* vm, jclass cls, jobject inst)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    if (!cls && inst)
    {
        cls = vm->GetObjectClass(inst);
    }
    
    this->vm = vm;
    this->cls = cls;
    this->inst = vm->NewGlobalRef(inst);
}

ZipEntry::ZipEntry(JVM* vm, const std::string &name)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    jstring str = vm->NewStringUTF(name.c_str());
    
    this->vm = vm;
    this->cls = vm->FindClass("java/util/zip/ZipEntry");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "(Ljava/lang/String;)V");
    jobject localInstance = vm->NewObject(this->cls, method, str);
    this->inst = vm->NewGlobalRef(localInstance);
}

ZipEntry::ZipEntry(JVM* vm, ZipEntry* e)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    if (!e)
    {
        throw std::runtime_error("Invalid ZipEntry instance.");
    }
    
    this->vm = vm;
    this->cls = vm->FindClass("java/util/zip/ZipEntry");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "(Ljava/util/zip/ZipEntry;)V");
    jobject localInstance = vm->NewObject(this->cls, method, e->inst);
    this->inst = vm->NewGlobalRef(localInstance);
}
...
{
    jmethodID method = this->vm->GetMethodID(this->cls, "clone", "()Ljava/util/zip/ZipEntry;");
    jobject localInstance = this->vm->CallObjectMethod(this->inst, method);
    return new ZipEntry(this->vm, nullptr, localInstance);
}

std::string ZipEntry::getComment()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getComment", "()Ljava/lang/String;");
    jstring localInstance = reinterpret_cast<jstring>(this->vm->CallObjectMethod(this->inst, method));
    const char *nativeString = this->vm->GetStringUTFChars(localInstance, 0);
    std::string result = std::string(nativeString);
    this->vm->ReleaseStringUTFChars(localInstance, nativeString);
...
long ZipEntry::getCompressedSize()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getCompressedSize", "()J");
    return this->vm->CallLongMethod(this->inst, method);
}

long ZipEntry::getCrc()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getCrc", "()J");
    return this->vm->CallLongMethod(this->inst, method);
}

std::vector<unsigned char> ZipEntry::getExtra()
{
    std::vector<unsigned char> result;
    jmethodID method = this->vm->GetMethodID(this->cls, "getExtra", "()[B");
    jbyteArray bytes = reinterpret_cast<jbyteArray>(this->vm->CallObjectMethod(this->inst, method));
    result.resize(this->vm->GetArrayLength(bytes));
    void* arr = this->vm->GetPrimitiveArrayCritical(bytes, nullptr);
    memcpy(&result[0], arr, result.size());
...
int ZipEntry::getMethod()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getMethod", "()I");
    return this->vm->CallIntMethod(this->inst, method);
}

std::string ZipEntry::getName()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getName", "()Ljava/lang/String;");
    jstring localInstance = reinterpret_cast<jstring>(this->vm->CallObjectMethod(this->inst, method));
    const char *nativeString = this->vm->GetStringUTFChars(localInstance, 0);
    std::string result = std::string(nativeString);
    this->vm->ReleaseStringUTFChars(localInstance, nativeString);
...
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getSize", "()J");
    return this->vm->CallLongMethod(this->inst, method);
}

long ZipEntry::getTime()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "getTime", "()J");
    return this->vm->CallLongMethod(this->inst, method);
}

int ZipEntry::hashCode()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "hashCode", "()I");
    return this->vm->CallIntMethod(this->inst, method);
}

bool ZipEntry::isDirectory()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "isDirectory", "()Z");
    return this->vm->CallBooleanMethod(this->inst, method);
}



JarEntry::JarEntry(JVM* vm, jclass cls, jobject inst) : ZipEntry(vm, cls, inst)
{
    
}

JarEntry::JarEntry(JVM* vm, const std::string &name) : ZipEntry(vm, nullptr, nullptr)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    this->vm = vm;
    this->cls = vm->FindClass("java/util/jar/JarEntry");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "(Ljava/lang/String;)V");
    jobject localInstance = vm->NewObject(this->cls, method, str);
    this->inst = vm->NewGlobalRef(localInstance);
}

JarEntry::JarEntry(JVM* vm, ZipEntry* e) : ZipEntry(vm, nullptr, nullptr)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
...
JarEntry* JarEntry::clone()
{
    jmethodID method = this->vm->GetMethodID(this->cls, "clone", "()Ljava/util/jar/JarEntry;");
    jobject localInstance = this->vm->CallObjectMethod(this->inst, method);
    return new JarEntry(this->vm, nullptr, localInstance);
}
...
JarInputStream::JarInputStream(JVM* vm, InputStream* in)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    this->vm = vm;
    this->cls = vm->FindClass("java/util/jar/JarInputStream");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "(Ljava/io/InputStream;)V");
    jobject localInstance = vm->NewObject(this->cls, method, in->inst);
    this->inst = vm->NewGlobalRef(localInstance);
}

JarInputStream::JarInputStream(JVM* vm, InputStream* in, bool verify)
{
    if (!vm)
    {
        throw std::runtime_error("Invalid JVM instance.");
    ...
    this->vm = vm;
    this->cls = vm->FindClass("java/util/jar/JarInputStream");
    jmethodID method = vm->GetMethodID(this->cls, "<init>", "(Ljava/io/InputStream;Z)V");
    jobject localInstance = vm->NewObject(this->cls, method, in->inst, verify);
    this->inst = vm->NewGlobalRef(localInstance);
}

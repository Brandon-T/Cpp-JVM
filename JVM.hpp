...
public:
    ...
    virtual void close();
    virtual void flush();
    virtual void write(unsigned char* b, int bufferLength);
    virtual void write(unsigned char* b, int bufferLength, int off, int len);
...
{
public:
    ByteArrayInputStream(JVM* vm, unsigned char* buffer, int bufferLength);
    ByteArrayInputStream(JVM* vm, unsigned char* buffer, int bufferLength, int offset, int length);
...
{
public:
    ByteArrayOutputStream(JVM* vm);
    ByteArrayOutputStream(JVM* vm, int size);
...
protected:
    ...
    jobject inst;
    JVM* vm;
    ZipEntry(JVM* vm, jclass cls, jobject inst);
    
    friend JarEntry;
    friend JarInputStream;
    
public:
    ZipEntry(JVM* vm, const std::string &name);
    ZipEntry(JVM* vm, ZipEntry* e);
    virtual ~ZipEntry();
    ...
    long getCrc();
    std::vector<unsigned char> getExtra();
    int getMethod();
    std::string getName();
    long getSize();
    long getTime();
...
{
private:
    JarEntry(JVM* vm, jclass cls, jobject inst);
    
    friend JarInputStream;
    
public:
    JarEntry(JVM* vm, const std::string &name);
    JarEntry(JVM* vm, ZipEntry* e);
    virtual ~JarEntry();
...
{
public:
    JarInputStream(JVM* vm, InputStream* in);
    JarInputStream(JVM* vm, InputStream* in, bool verify);
...
protected:
    ZipEntry createZipEntry(const std::string &name);
};
